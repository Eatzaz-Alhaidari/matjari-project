                ...
                <div class="p-6 bg-white border-b border-gray-200">
                    ...
                    <div class="mb-6">
                        <form action="{{ route('admin.stores.index') }}" method="GET" class="flex items-center">
                            <input type="text" name="search" placeholder="ابحث باسم المتجر أو اسم المالك..." class="w-full md:w-1/3 border-gray-300 rounded-lg shadow-sm" value="{{ request('search') }}">
                    ...
                    <div class="overflow-x-auto bg-white">
                        <table class="min-w-full">
                            <thead class="bg-brand-blue-50">
                                <tr>
                                    <th class="px-5 py-3 text-right text-xs font-bold text-brand-blue-800 uppercase tracking-wider">اسم المتجر</th>
                                    <th class="px-5 py-3 text-right text-xs font-bold text-brand-blue-800 uppercase tracking-wider">المالك (البائع)</th>
                                    ...
                                    <th class="px-5 py-3 text-right text-xs font-bold text-brand-blue-800 uppercase tracking-wider">تاريخ الإنشاء</th>
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @forelse ($stores as $store)
                                    <tr class="hover:bg-gray-50">
                                        <td class="px-5 py-4">
                                            ...
                                            <div class="text-xs text-gray-500">{{ $store->address ?? 'لا يوجد عنوان' }}</div>
                                        </td>
                                        <td class="px-5 py-4">
                                            <div class="text-sm font-medium text-gray-900">{{ $store->user?->name ?? 'غير معروف' }}</div>
                                        ...
                                        <td class="px-5 py-4 text-center text-sm font-medium">
                                            <div class="flex items-center justify-center space-x-2 space-x-reverse">
                                                <form action="{{ route('admin.stores.toggleStatus', $store->id) }}" method="POST" class="inline-block">
                                                    @csrf
                                                    <button type="submit" class="px-2 py-1 text-xs rounded-full font-semibold {{ $store->is_active ? 'bg-yellow-100 text-yellow-800 hover:bg-yellow-200' : 'bg-green-100 text-green-800 hover:bg-green-200' }}">
                                                        {{ $store->is_active ? 'تعطيل' : 'تفعيل' }}
                                                    </button>
                                                </form>
                                                <a href="{{ route('admin.stores.edit', $store->id) }}" class="px-2 py-1 text-xs rounded-full font-semibold bg-blue-100 text-blue-800 hover:bg-blue-200">تعديل</a>
                                                <form action="{{ route('admin.stores.destroy', $store->id) }}" method="POST" onsubmit="return confirm('هل أنت متأكد؟ حذف المتجر سيؤدي إلى بقاء البائع بدون متجر.');" class="inline-block">
                                                    @csrf
                                                    @method('DELETE')
                                ...
                                @empty
                                    <tr><td colspan="6" class="px-6 py-4 text-center text-gray-500">لا توجد متاجر حالياً.</td></tr>
                                @endforelse
                            </tbody>
                        </table>
                    </div>
