                    ...
                    <form action="{{ route('admin.vendors.update', $vendor->id) }}" method="POST"
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div class="md:col-span-2">
                                <h3 class="text-lg font-bold text-brand-blue-800 border-b pb-2 mb-4">معلومات البائع
                                    الشخصية</h3>
                            </div>
                            ...
                            <div>
                                <label for="email" class="block font-medium text-sm text-gray-700">البريد
                                    الإلكتروني</label>
                            ...
                            <div>
                                <label for="store_name" class="block font-medium text-sm text-gray-700">اسم
                                    المتجر</label>
                                <input id="store_name" type="text" name="store_name"
                                    value="{{ old('store_name', $vendor->store?->name) }}"
                                    class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                                <x-input-error :messages="$errors->get('store_name')" class="mt-2" />
                            </div>

                            <div>
                                <label for="commercial_registration" class="block font-medium text-sm text-gray-700">رقم
                                    السجل التجاري</label>
                                <input id="commercial_registration" type="text" name="commercial_registration"
                                    value="{{ old('commercial_registration', $vendor->store?->commercial_registration) }}"
                                    class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                                <x-input-error :messages="$errors->get('commercial_registration')" class="mt-2" />
                            </div>

                            <div class="md:col-span-2">
                                <label for="address" class="block font-medium text-sm text-gray-700">عنوان
                                    المتجر</label>
                                <textarea id="address" name="address" rows="3"
                                    class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">{{ old('address', $vendor->store?->address) }}</textarea>
                            ...
                            <div class="md:col-span-2">
                                ...
                                <textarea id="description" name="description" rows="4"
                                    class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">{{ old('description', $vendor->store?->description) }}</textarea>
                        ...
                        <div class="flex items-center justify-end mt-8 border-t pt-6">
                            <a href="{{ route('admin.vendors.index') }}"
                                class="px-5 py-2 bg-gray-100 text-gray-700 font-semibold rounded-lg shadow-sm hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-gray-300 focus:ring-offset-2">
                                إلغاء
                            </a>
