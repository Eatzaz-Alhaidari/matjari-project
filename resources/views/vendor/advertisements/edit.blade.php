<x-vendor-layout>
    <x-slot name="title">
        تعديل الإعلان: {{ $advertisement->title }}
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <h2 class="text-2xl font-bold text-black mb-6">تعديل الإعلان</h2>

                    <form action="{{ route('vendor.advertisements.update', $advertisement->id) }}" method="POST"
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <x-text-input id="title" class="block mt-1 w-full" type="text" name="title"
                                    :value="old('title', $advertisement->title)" required autofocus />
                            ...
                            <div>
                                ...
                                <select id="status" name="status"
                                    ...
                                    <option value="pending" {{ old('status', $advertisement->status) == 'pending' ? 'selected' : '' }}>في الانتظار</option>
                                    <option value="active" {{ old('status', $advertisement->status) == 'active' ? 'selected' : '' }}>نشط</option>
                                    <option value="inactive" {{ old('status', $advertisement->status) == 'inactive' ? 'selected' : '' }}>معطل</option>
                                </select>
                            ...
                            <div>
                                ...
                                <x-text-input id="start_date" class="block mt-1 w-full" type="date" name="start_date"
                                    :value="old('start_date', $advertisement->start_date->format('Y-m-d'))" required />
                            ...
                            <div>
                                ...
                                <x-text-input id="end_date" class="block mt-1 w-full" type="date" name="end_date"
                                    :value="old('end_date', $advertisement->end_date->format('Y-m-d'))" required />
                            ...
                            <div>
                                ...
                                <x-text-input id="budget" class="block mt-1 w-full" type="number" step="0.01"
                                    name="budget" :value="old('budget', $advertisement->budget)" required />
                            ...
                            <div class="md:col-span-2">
                                ...
                                @if($advertisement->image)
                                    <div class="mb-2">
                                        <img src="{{ asset('storage/' . $advertisement->image) }}" alt="Current Image"
                                ...
                                <p class="text-xs text-gray-500 mt-1">اتركه فارغاً إذا كنت لا تريد تغيير الصورة. الحد
                                    الأقصى للحجم 2 ميجابايت</p>
                        ...
                        <div class="mt-6">
                            ...
                            <textarea id="description" name="description" rows="4"
                                class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm"
                                placeholder="اكتب وصفاً مفصلاً للإعلان...">{{ old('description', $advertisement->description) }}</textarea>
