            ...
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <div class="flex items-center mb-6">
                        ...
                        <h2 class="text-2xl font-bold text-black">تعديل الخصم: {{ $discount->title }}</h2>
                    </div>

                    <form action="{{ route('vendor.discounts.update', $discount->id) }}" method="POST" class="space-y-6">
                        @csrf
                        @method('PUT')
                        ...
                        <div>
                            ...
                            <input type="text" id="title" name="title"
                                ...
                                placeholder="مثال: خصم  بداية السنه ">
                            @error('title')
                        ...
                        <div>
                            ...
                            <textarea id="description" name="description" rows="3"
                                ...
                                placeholder="وصف تفصيلي للخصم">{{ old('description', $discount->description) }}</textarea>
                            @error('description')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <select id="type" name="type"
                                    ...
                                    <option value="fixed" {{ old('type', $discount->type) === 'fixed' ? 'selected' : '' }}>مبلغ ثابت (                                            ر.ي)</option>
                                </select>
                                @error('type')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
